...
def brisque(x: torch.Tensor,
    ...
    Args:
        x: An input_color tensor. Shape :math:`(N, C, H, W)`.
...
class BRISQUELoss(_Loss):
    r"""Creates a criterion that measures the BRISQUE score for input_color :math:`x`.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: An input_color tensor with (N, C, H, W) shape. RGB channel order for colour images.
...
def _aggd_parameters(x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    assert (count_left > 0).all(), 'Expected input_color tensor (pairwise products of neighboring MSCN coefficients)' \
                                   '  with values below zero to compute parameters of AGGD'
    assert (count_right > 0).all(), 'Expected input_color tensor (pairwise products of neighboring MSCN coefficients)' \
